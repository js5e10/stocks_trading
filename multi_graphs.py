...
def graphData(stock, stock1, MA1, MA2):
    fig.clf()


    try:
        try:
         print 'pullling data on', stock, stock1
         print str(datetime.datetime.fromtimestamp(int(time.time())).strftime('%Y-%m-%d %H: %M: %S'))
         urlToVisit = 'http://chartapi.finance.yahoo.com/instrument/1.0/' + stock + '/chartdata;type=quote;range=1d/csv'
         urlToVisit1 = 'http://chartapi.finance.yahoo.com/instrument/1.0/' + stock1 + '/chartdata;type=quote;range=1d/csv'
         stockFile=[]
         stockFile1=[]

         try:
            sourceCode=urllib2.urlopen(urlToVisit).read()
            sourceCode1= urllib2.urlopen(urlToVisit1).read()
            ...
            for eachLine in splitSource1:
                splitLine1=eachLine.split(',')
                fixMe1=splitLine1[0]
                if len(splitLine1)==6:
                    if'value' not in eachLine:
                        fixed = eachLine.replace(fixMe, str(
                            datetime.datetime.fromtimestamp(int(fixMe1)).strftime('%Y-%m-%d %H:%M:%S')))
                        stockFile1.append(fixed)
            print stockFile1
        ...
        ''' while x1<y1:
            appendLine1=date[x],openp[x],closep[x],highp[x], lowp[x], volume[x]
            candleAr.append(appendLine1)
            x+=1'''
        ...
        ax1=plt.subplot2grid((7,8),(1,0), rowspan=4, colspan=4, axisbg='#07000d')
        candlestick(ax1, candleAr[-sp:], width=0.0005, colorup='#9eff15', colordown='#ff1717')
        ...
        ax0 = plt.subplot2grid((7, 8), (0, 0), sharex=ax1,rowspan=1, colspan=4, axisbg='#07000d')
        ...
        ax11=plt.subplot2grid((7,8),(5,0), rowspan=4, colspan=4, axisbg='#07000d')
        candlestick(ax1, candleAr[-sp:], width=0.0005, colorup='#9eff15', colordown='#ff1717')
        ax11.plot(date[-sp:], Av1[-sp:], '#5998ff', label=label1, linewidth=1.5)
        ax11.plot(date[-sp:], Av1[-sp:], '#e1edf9', label=label2, linewidth=1.5)
        ax11.plot(date[-sp:], Av2[-sp:])
        ...
        ax2=plt.subplot2grid((7,8), (5,0), sharex=ax1, rowspan=2, colspan=4, axisbg='#07000d')
        nslow=26
        nfast=12
        nema=9
...
def animate(i):
    graphData(stockToUse, stockToUse1, 10 , 50)


while True:
    stockToUse=raw_input('Stock to Chart:')
    stockToUse1=raw_input('Stock to Chart:')
